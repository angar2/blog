...
const MobileNav = ({
  ...
  return (
    <nav
      className={`sm:hidden fixed transform ${
    ...
    >
      ...
      <div className="fixed bottom-8 self-end mr-4">
        <div className="flex flex-col items-end">
          <div className="flex items-center">
            <Link
              href="/experience"
              className="h-fit text-xs font-semibold transition-colors duration-300 hover:text-[#008B6B] sm:text-base"
              onClick={() => setIsMenuOpen(false)}
            >
              Resume
            </Link>
            <div className="w-px bg-gray-300 h-4 mx-2"></div>
            <Link href={GITHUB_PROFILE_URL} className="self-center">
              <svg
              ...
              >
                <path
                  ...
                  className="fill-[#2C2C2C] transition-colors duration-300 hover:fill-[#9D4AD1]"
                ></path>
              </svg>
            </Link>
          </div>
          <div className="mt-3">
            <h3 className="text-xs tracking-tighter leading-tight text-end">
