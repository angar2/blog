...
description: 백엔드 서버에서 이미지 파일 저장 시 원격 서버 배포 후 발생한 디렉토리 권한 오류를 리눅스 권한 체계로 분석하고, 디렉토리 소유자 변경으로 해결한 과정을 정리한다.
track: notes
created: 2024-01-15T09:17
tags: [linux, ubuntu, permissions]
sources:
  - title: 🐧 리눅스 파일 & 디렉토리 권한 (소유권 / 허가권 / 특수권한)
    url: https://inpa.tistory.com/entry/LINUX-📚-파일-권한-소유권허가권-💯-정리
  - title: 리눅스 권한이란 무엇인가? (리눅스 소유권, 리눅스 허가권, 리눅스 권한)
    url: https://blog.naver.com/haejoon90/220742416258
---

백엔드 서버 개발 중, 회원의 프로필이나 게시물 이미지 파일을 서버에 저장해야하는 기능을 작업중이다.

이미지 파일 저장을 위해 S3와 같은 클라우드 스토리지를 사용할 수 있지만 서버 내부에 직접 저장하는 방식을 먼저 진행해보고자 했다.

기능 구현 후, 로컬 개발환경에서는 잘 작동하던 것이 원격 클라우드 환경에 배포한 후에는 권한 이슈가 발생해 기능을 작동하지 않았다.

## Issue: 디렉토리 사용 권한 오류

이미지 파일 저장 기능에서 발생한 디렉토리 권한 오류 상황과 원인이 되는 로직을 살펴본다.
...
권한 오류를 해결하기 위해 먼저 리눅스의 파일 및 디렉토리 권한 체계를 정리한다.
...
![root 디렉토리 하위 폴더 권한 목록](/assets/images/blog/17/1.png)
...
위에서 정리한 권한 체계를 바탕으로 실제 문제를 해결한 과정을 정리한다.
...
![디렉토리 소유자 변경 후 권한 목록](/assets/images/blog/17/2.png)
