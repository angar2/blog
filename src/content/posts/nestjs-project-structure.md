---
title: Nest.js 프로젝트 생성과 구조
description: Node.js 웹서버 프레임워크 Nest.js의 설치와 프로젝트 생성, 디렉토리 구조, 그리고 데코레이터·의존성 주입·예외 처리 등 주요 기능을 정리한다.
track: notes
created: 2023-12-31T09:00
tags: [nestjs]
sources:
  - title: Nest.js 공식 문서
...
Node.js를 공부하면서 웹서버 프레임워크를 알아보던 중 **Nest.js**를 알게 되었다. Express.js와는 달리 프레임워크의 무게감이 있지만, **Nest.js**만의 철저(?)한 규칙과 다양한 내부 기능으로 다른 Node.js 웹서버 프레임워크들과 비교해 서버 개발에 안정감과 코드관리가 용이하다는 특징이 있다고 한다.
...
`nestjs/cli`는 Nest.js 관련 명령어를 실행할 수 있는 ‘커맨드 라인 인터페이스’ 도구이다. 이 도구를 이용해 터미널에서 “nest”라는 명령어를 사용할 수 있다.
...
_project-name_ 에 입력한 이름으로 프로젝트 디렉토리가 생성되고 동시에 아래 리스트의 패키지가 모두 설치된다. 설치된 Nest.js의 버전은 `v9.3.9`이다.
...
![Nest.js 디렉토리 구조](/assets/images/blog/1/1.png)
...
Nest.js CLI에는 **generate**라는 파일 생성 명령어가 있다. 파일의 역할에 맞는 코드가 미리 작성되어 있는 파일을 생성해 빠르게 코드의 프레임 구조를 잡을 수 있다.
...
하나의 앱은 기본적으로 `Module`, `Controller`, `Service` 구조로 이루어진다. **nest generate**를 이용해 각각의 파일을 생성해 앱을 구성해보자.
