---
title: Nest.js 의존성 주입 개념과 적용 방법
description: 객체 지향 프로그래밍 관점에서 의존성 주입의 개념과 인스턴스 생성 방식과의 차이를 정리하고, Nest.js의 Controller-Service 구조에서 의존성 주입이 실제로 어떻게 적용되는지 살펴본다.
track: notes
created: 2024-01-02T09:00
tags: [nestjs, dependency-injection]
sources:
  - title: 객체지향 프로그래밍이란?
    url: https://jongminfire.dev/객체지향-프로그래밍이란
  - title: 개발자 면접 단골질문 객체지향 프로그래밍에 대해 알아보자 | oop 한번에 설명해드림
    url: https://www.youtube.com/watch?v=fECCYukfVok
---
...
> - **Nest.js**는 프로토타입 기반의 OOP를 지원하는 JavaScript로 만들어진 프레임워크이지만, TypeScript를 이용해 클래스 기반으로 객체를 생성하고 의존성 주입을 주된 방식으로 활용한다.

> [!tip]
> 객체 지향 프로그래밍에 대한 배경지식은 [객체지향 프로그래밍이란?](https://jongminfire.dev/객체지향-프로그래밍이란)과 [개발자 면접 단골질문 객체지향 프로그래밍에 대해 알아보자 | oop 한번에 설명해드림](https://www.youtube.com/watch?v=fECCYukfVok)을 참고했다.
...
가령, 단순한 계산 로직 혹은 해당 클래스에서 필수로 사용해야하는 경우에는 인스턴스를 직접 생성해 사용하고,
변동성이 있거나 큼지막한 모듈 간의 의존성을 필요로 할 경우에는 의존성 주입을 사용하는 것이 효율적일 수 있다.

## Nest.js의 의존성 주입

Nest.js를 처음 접하면서 가장 어색했던 점은 다른 파일에 있는 메소드를 가져다 쓸 때, import 후 바로 메소드를 사용하는 것이 아니라 마치 가져온 기능을 장착하듯 `constructor`에 먼저 설정을 해주어야 한다는 점이었다. 그건 내가 가져온 기능이 **Class**였기 때문이다.
