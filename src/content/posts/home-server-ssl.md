---
title: Nginx SSL/TLS 발급과 자동 갱신
description: Cloudflare DNS-01 챌린지와 Certbot으로 Let's Encrypt 인증서를 발급하고 Nginx에 적용해 자동 갱신까지 설정하는 과정을 정리한다.
track: log
created: 2024-10-23T09:52
tags: [home-server, nginx, ssl]
sources:
  - title: Let's encrypt SSL 인증서 자동 갱신하기 (Cloudflare) - j911
    url: https://j911.me/letsencrypt-ssl-certification/
---
...
- **Cloudflare API 토큰 발급**
  ...
  ![Cloudflare 사용자 API 토큰 페이지에서 토큰 생성 버튼이 강조된 화면](/assets/images/blog/52/1.png)

  ![사용자 설정 토큰 생성 화면에서 권한을 영역 - SSL 및 인증서 - 편집으로 지정하는 화면](/assets/images/blog/52/2.png)

  ![생성된 영역 DNS 편집 API 토큰이 활성 상태로 목록에 표시된 화면](/assets/images/blog/52/3.png)
...
- **서버 환경변수로 Cloudflare API 토큰 설정**
  ...
  글로벌 API 키를 사용하는 것이 아닌 특정 용도의 API 토큰을 발행해서 사용했을 경우, dns_cloudflare_api_key 및 dns_cloudflare_email이 아닌 **dns_cloudflare_api_token**으로 변수명을 설정해야 한다.
...
- **패키지 및 플러그인 설치**
  ...
  > [!note]
  > - certbot: SSL/TLS 인증서를 발급하고 관리하는 툴
