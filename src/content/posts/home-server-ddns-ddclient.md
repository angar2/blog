---
title: ddclient으로 동적 DNS(DDNS) 자동화
description: DNS 동적 업데이트 클라이언트 ddclient를 설치하고 Cloudflare 연동 환경변수를 설정해 동적 IP를 자동으로 갱신하는 과정을 정리한다.
track: log
created: 2024-10-31T09:58
tags: [home-server, ddns, ddclient]
---

이전 문서에서 DDNS가 이루어지는 과정을 테스트해보았다. 해당 과정은 DNS 동적 업데이트 클라이언트인 **`ddclient`**가 수행하는 과정과 동일하다.
...
Cron 작업과 통합되어 주기적으로 IP 주소를 확인하기 때문에 별도로 타이머나 스케줄링 스크립트를 운영할 필요가 없으며, 다양한 DNS 서비스를 지원하기 때문에 DNS 서비스를 바꾸더라도 업데이트 로직을 직접 수정할 필요없이 간단한 설정 변경만으로 관리할 수 있다.

## ddclient 설치 및 설정

ddclient 설치부터 환경변수 설정, 서비스 실행까지 순서대로 진행한다.
