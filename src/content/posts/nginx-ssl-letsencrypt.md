---
title: Nginx HTTPS - Let's Encrypt SSL/TLS 설정
description: 원격 서버에 Let's Encrypt certbot으로 SSL/TLS 인증서를 발급받고, 이를 Nginx 서버 블록에 적용해 HTTPS 리버스 프록시를 구성하는 과정을 정리한다.
track: notes
created: 2024-01-12T09:15
tags: [nginx, ssl, lets-encrypt]
sources:
  - title: Google 관리 콘솔 도구 상자
    url: https://toolbox.googleapps.com/apps/dig/#TXT/_acme-challenge.dailyrecord.store
  - title: SSL Server Test
    url: https://www.ssllabs.com/ssltest/
---

원격 서버에 HTTPS를 적용하기 위해 SSL/TLS를 발급받아야 하는데, **무료**로 SSL/TLS를 발급해주는 공개 인증 기관(CA)인 Let's Encrypt의 `certbot`을 이용하기로 했다.

## SSL/TLS 발급

Let's Encrypt의 `certbot`으로 도메인 소유권을 확인하고 인증서를 발급받는 과정을 정리한다.
...
![certbot SSL 인증서 발급](/assets/images/blog/15/1.png)

발급 명령어를 실행하면, DNS TXT 레코드에 입력할 `value`가 주어진다. 여기서 바로 Enter를 클릭해 넘어 가면 안 되고, 레코드 등록 완료를 확인한 후 넘어가야 한다.
...
![certbot SSL 인증서 발급](/assets/images/blog/15/2.png)

### 레코드 등록

현재 도메인은 가비아의 도메인을 이용 중이므로, '가비아 도메인 설정'에서 해당 도메인에 TXT 레코드를 생성한다.

![TXT 레코드 등록](/assets/images/blog/15/3.png)
...
![Google 관리 콘솔 도구 상자](/assets/images/blog/15/4.png)

등록이 확인되면 터미널에서 Enter를 클릭해 발급을 완료한다.

![certbot SSL 인증서 발급](/assets/images/blog/15/5.png)
...
그렇게 **HTTPS 443 포트**로 접근할 경우, SSL을 진행하고 요청된 서브 도메인에 따라 백엔드(`api.`)와 프론트엔드(`www.`)를 각각 연결해주면 최종적으로 HTTPS로 리버스 프록시를 적용해 이용할 수 있다.
...
![HTTPS](/assets/images/blog/15/6.png)
...
![SSL 보안 점수 체크](/assets/images/blog/15/7.png)
